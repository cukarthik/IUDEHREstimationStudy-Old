...
createTcos <- function(outputFolder) {
  pathToCsv <- system.file("settings", "TcosOfInterest.csv", package = "IUDCLW")
...
getAllControls <- function(outputFolder) {
  ...
  if (file.exists(allControlsFile)) {
    ...
    pathToCsv <- system.file("settings", "NegativeControls.csv", package = "IUDCLW")
...
runCohortCharacterization <- function(connectionDetails,
  ...
  FeatureExtraction::saveCovariateData(covariateData, paste0(cohortId,"_covariates"))
